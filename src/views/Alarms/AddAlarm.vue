<template>
	<div class="add-alarm pb-2">
		...
		<div class="row has-header">
			<form class="col mx-2">
				<div>Time:</div>
				<b-form-timepicker class="mb-2" placeholder="Time" v-model="time"></b-form-timepicker>
				<div>Label:</div>
				<b-form-input class="mb-2" placeholder="Label" v-model="title"></b-form-input>
				<div>Notes:</div>
...
export default class AddAlarm extends Vue {
	...
	get daySelected() {
		return this.alarm.recurringDaily;
	}

	set daySelected(value) {
		this.alarm.recurringDaily = value;
	}
	...
	saveAlarm() {
		if (this.isEdit) {
			this.$store
				.dispatch("updateAlarm", { id: this.$route.params.id, time: this.alarm.time, title: this.alarm.title, notes: this.alarm.notes, snoozeInterval: this.alarm.snoozeInterval, recurringDaily: this.alarm.recurringDaily, recurringMonthly: this.alarm.recurringMonthly, recurringYearly: this.alarm.recurringYearly, days: this.alarm.days})
				.then(() => {
			...
			this.$store
				.dispatch("addAlarm", { time: this.alarm.time, title: this.alarm.title, notes: this.alarm.notes, snoozeInterval: this.alarm.snoozeInterval, recurringDaily: this.alarm.recurringDaily, recurringMonthly: this.alarm.recurringMonthly, recurringYearly: this.alarm.recurringYearly, days: this.alarm.days })
				.then(() => {
					this.$router.replace("/alarms");
				});
		}
