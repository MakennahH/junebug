...
export default class Calendar extends Vue {
	...
	get upcomingEvents() {
		return this.events.filter((event: any) => (moment(event.date) >= moment() && moment(event.date) <= moment().add(1, "months")) || this.isWeeklyRecurring(event) || event.recurringMonthly);
	}
